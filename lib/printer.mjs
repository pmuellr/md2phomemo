...
async function printOn(printer, deferred, data, height, width, binFile) {
  ...
  log(`connected to printer, writing ${buffer.length} bytes`)
  ...
  characteristic.write(byteBuffer, false, async (err) => {
    ...
    // will need to find out if there's something I can read, or an 
    // event, to tell when the printer is done printing. Need to wait
    // longer for bigger jobs.
    const waitTime = Math.ceil(height / 100)
    log(`disconnecting from printer in ${waitTime} seconds`)
